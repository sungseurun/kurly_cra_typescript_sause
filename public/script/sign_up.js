(($, window)=>{  //화살표 함수 (function 생략 함수)
   ...
   const SignUp = {
      회원:{
         ...
         휴대폰인증확인: false, // bool
         휴대폰인증번호: '',   // string
         setId: 0,   // setInterval 셋인터발 변수 // number
         주소1: '',  // string
         주소2: '',  // string
         성별: '',   // string
         생년: '',   // string
      ...
      ////////////// 아이디, 이메일 중복검사를 위한 데이터베이스 가져오기 ////////////   

         // 1. 데이터베이스 서버에서 회원 데이터 가져오기 
         // 2. 아이디 중복 검사
         // tmfjs5902.dothome.co.kr/kurly_1110/
         // tmfjs5902.dothome.co.kr/myadmin/
         $.ajax({
            url:'./select.php',
            type:'GET',
      ...
      idFn(){  // 아이디
         // 1. 특수문자 입력과 동시에 삭제 replace(정규식,'')
         // 2. 6자 이상 16자 이하의영문 혹은 영문과 숫자를 조합 (역슬래쉬 문자로 출력시 역슬래쉬 두번, 각괄호와 중괄호 사이에 역슬래쉬 넣어줘야 문자로 인식함)
         ...
         $('#id').on({
            keyup: function(){
               ...
               // \s 공백
               // \S 공백아닌것(공백 그 외) ===[^\s]
               // + 글자 1자 이상 ~ 여러글자 (무조건 1자 이상)
               // * 글자 0자 이상 ~ 여러글자 (와도되고 안와도 되고)
               // ? 글자 0자 또는 1자
               // . 글자 1자
               // | or (이거나, 또는)
               // & and (이고, 그리고, 이면서)
               ...
               // . 1자
               // (.)\1 동일한 글자 2자
               // (.)\1\1 동일한 글자 연속사용 글자 3자
     
               // (?=.*[A-Za-z])+  영문대소문자 1자이상 포함
               // (?=.*[0-9])+  숫자 1자이상 포함
               // (?=.*[\d])+  숫자 1자이상 포함
               // (?=.*[가-힣]ㄱ-ㅎㅏ-ㅣ)+  한글 1자이상 포함
               // (?=.*[`~!@#$%^&*()\-_=+|\\\]\}\[\{'";:/?.>,<])+  특수문자 1자이상 포함
               
     
               // 1. 특수문자는 입력과 동시에 삭제
               // 2. 글자수는 6 ~ 16 조건 범위 또는 영문 대소문자 범위
               // 3. 입력상자가 공백이면 에러 메시지 안띄우기
               // 4. 글자 사이 공백 허용 안함
               // 특수문자를 문자로 인식하게 하는 방법 => \ (역슬래쉬)를 찍어야함
               // 특수문자 브랏켓 기호 안에 표기 방식 => \ - 마이너스 부호 \\ 역슬래쉬 \{ \} \[ \]
     
               regExp1 = /[`~!@#$%^&*()\-_=+|\\\]\}\[\{'";:/?.>,<]/g;   //특수문자   // g =? global(전체)
               regExp2 = /.{6,16}/g;   // 글자수는 6 ~ 16
               regExp3 = /(?=.*[A-Za-z])+(?=.*[0-9])*/g;  // 영문(대소문자) 필수 포함, 숫자 선택 포함 : ()+()* ==>  영문 (?=.*[A-Za-z])+ 또는 숫자 (?=.*[0-9])*
               regExp4 = /\s/g;   // 공백문자
              ...
              // 2. 입력제한 조건문(2,3,4,조건문)
               if(idVal===''){   // 공백이면
                  ...
                  if( regExp2.test(idVal) === false  ||  regExp3.test(idVal) === false || regExp4.test(idVal) === true ){
                     // console.log( '6자이상 16자 이하로 입력하세요' );
               ...
               // result = regExp1.test( idVal );   => 정규표현식 테스트 점검 결과
      ...
      pwFn(){  // 비밀번호
         ...
         $('#pw').on({
            keyup: function(){
               ...
               // const regExp4 = /(\w)\1\1/g;  //동일한 글자 연속3번 사용 불가 
               ...
               // 예문1]  tmfjs12345   영문숫자
               // 예문2]  tmfjs!@#$%   영문특수문자
               // 예문3]  1234567tmfjs  숫자영문
               // 예문4]  123456789#$%@   숫자특수문자
               // 예문5]  !@#$%^&*456789  특수문자문자
               // 예문6]  tmfjs 12345  문자 공백 숫자  [오류]
               // 예문7]  tmfjs12345@  문자숫자특수문자 
               // 예문8]  tmfjs134@@@  문자숫자특수문자연속3자이상 [오류]
         ...
         // 1. 빈값이면 : 비밀번호를 한 번 더 입력해 주세요. 메시지 오류
         // 2. 두값을 동일여부 비교 : 동일한 비밀번호
         ...
         $('#pw2').on({
            keyup: function(){
               ...
               if( pw2Val===''){  // 입력값이 공백이면 ** 따옴표 사이에 간격두지말고 붙여쓰기**
      ...
      nameFn(){  // 이름
         ...
         $('#name').on({
            keyup: function(){
               ...
               // // 입력과 동시에 특수문자는 삭제
               // if( regExp.text( nameVal )=== true ){;  // 조건문 사용하는 경우 특수문자이면
      ...
      const that = this;
         ...
         $('#email').on({
            keyup: function(){
               ...
               // 이메일 정규식 :    @   . 필수포함
               // 사용불가 문자: @ ; : [ ] " , \
               // @ 뒷부분은 특수문자사용 불가 문자 숫자
               // 이메일 예문1 :  tmfjs5905@tmfjs5902.com
               // 이메일 예문2 :  tmfjs5905@tmfjs5902.co.kr
               // 이메일 예문3 :  tmfjs.123@naver.com
               // 이메일 예문4 :  tmfjs@naver.com
               // 이메일 예문5 :  tmfjs-#123@naver.com
               // 이메일 예문6 :  tmfjs-@!123@naver.com
               // 이메일 예문7 :  tmfjs-@!123@naver.12특수문자불가.com
               // ignore : 대소문자 무시
               // const regExp = /^ ( [A-Z0-9]+  [^@;:\[\]",\\]*  )+  @   [A-Z0-9]+  ([.\-_]?[A-Z0-9]+)*  .[A-Z]{2,3}  $/gi;              
               // 그룹1   @  그룹2 . 그룹3.그룹4()
               // 예] tmfjs5905@naver.co.kr
               // [시작영문숫자1자이상]@[영문숫자1자이상].([특수문자기본0자1자]?[영문숫자0자이상]*)*.[영문]{2,3}끝
               // /^[A-Z0-9]@[A-Z0-9]([._\-]?[A-Z0-9])*.[A-Z]{2,3}$/gi;
               // ([시작영문숫자1자이상][^공백])+@[영문숫자1자이상].([특수문자기본0자1자]?[영문숫자0자이상]*)*.[영문]{2,3}끝
   ...
   /////////////////////////////////////// /* 이메일 중복확인 버튼 이벤트 */ /////////////////////////////////////
      ...
      hpFn(){  // 휴대폰 
         ...
         // 휴대폰 
         // 0103486441   10자 이상이면 우측 버튼 보이기 클릭가능한 형태로 변경(disabled=false)  
         // 01079426441  11자
         // 숫자{3} [숫자]{3,4} [숫자]{4}
         // 010{3} [0-9]{3,4} [0-9]{4}
         // 011
   ...
   ////////////////////////////////* 인증번호 확인 버튼 클릭 이벤트 *///////////////////////////////
      ...
      ////////* 다른번호 인증 *////////
         $('.hp2-btn').on({
            click: function(e){
               e.preventDefault();
               $('#hp').attr('disabled', false)     // => 속성
                       .val('')                     // 입력상자 내용 삭제
                       .focus();                    // 입력대기 상태
               $('.hp .guid-text1').addClass('on'); // 안내 텍스트 출력
            }
         });
      ...
      /////* 카카오주소검색 API 함수 */////
         ...
         // 비교하고 만약 주소키 kurly Address가 있다면 유지
         function addressFn(){
            ...
            for(let i=0; i<sessionStorage.length; i++){
               if( sessionStorage.key(i) === addressKey ){
                  ...
                  key = sessionStorage.getItem(addressKey);          // 키 가져오기 주소2ㄱ밧 1408동 205호
         ...
         // 주소 검색하여 입력된 주소입력상자 내용 유지하기
         // 새로고침을 해도 그대로 유지 되어야한다.
         // 매시간동안 계속 유지
         // 로컬스토레이지 데이터 가져오기 : 키(전화번호)를 이용해서
         // 지금 현재 주소입력창이 열려있다면 아래의 내용 수행
         // function addrTimer(){
         //    alert('부모창 주소타이머함수 호출 실행!');
         //    if( $('#addr1').val() !== '' ){
         //       console.log('주소2');
         //       let localData = JSON.parse(localStorage.getItem( $('#addr2').val() ));
         ...
         //    if(  localStorage.key(i) === '1408동 205호' ){
         //       console.log( '해당주소를 찾았어요!' + i );   
         //    } 
         //    else {
         //       console.log( '해당하는 주소가 없어요!' + i  );   
         //    }
         // }
      ...
      genderFn(){  // 성별
         ...
         $('.gender-btn').each(function(idx, item){
            // console.log(idx, item.id, item.type, item.type, item.name, item.value);
            $(this).on({  // 객체 태그 요소 $('.gender-btn')
               change: function(){
                  // console.log(item.value)
                  that.회원.휴대폰 = $(this).val();   // 성별저장
                  // console.log( this );       // 객체 태그 요소 <input type="radio" id='female' class='gender' name='gender' value='여자'>
                  // console.log( that );       // 객체(object) SignUp
                  // console.log( that.획원 );  // 객체(object) SignUp.gender 속성값 출력
               }
            });
         });
      ...
      birthFn(){  // 생년월일
         ...
         /////////* 3항 연산자를 쓸 때  :(땡땡)은 계산이 맞으면~ 이란 뜻. | 는 또는 이란 뜻? */////////
         const regExpUnNum = /[^0-9]/g;  // 숫자가 아닌 것
   
         // 1. 생년 (1900년 ~ 1999년)  ~ (2000년 ~ 2999년)
         const regExpYear = /^(?:19(?:2[2-9]|[3-9][0-9])|2[0-9][0-9][0-9])$/g;
         
         // 2. 생월 01-09  ~  10 11 12  /  1-9   ~  10 11 12 
         const regExpMonth = /^(?:0?[1-9]|1[0-2])$/g;   // 또는  /^(?:0?[1-9]|10|11|12)$/g;
   
         // 3. 생일 1 ~ 31 / 01 ~ 31  ==>  1-9 10-19 20-29 30-31 / 01-09 10-19 20-29 30-31 
                  ...
                  if( $('#date').val()==='' ){
                     ...
                     if( regExpDate.test( $('#date').val().toString() ) === false ){
                        ...
                        if( Number($('#date').val()) > 31 ){
                           ...
                           that.회원.생일 = $('#date').val();
                           // that.회원.생년월일 = `${$('#year').val()}-${$('#month').val()}-${$('#date').val()}`;
                        } 
                     }
         ...
         $('.add-input-btn').each(function(idx,item){
            $(this).on({
               change: function(){                         
                  ...
                  // that.회원.추가입력사항 = `${item.value}: ${$('#choocheon').val()}`; 
               }
            })
         });
      ...
      /////* 전체 동의 합니다. 체크박스 이벤트 */////
         $('#chkAll').on({
            change: function(){
               ...
               // 비구조화 = 구조 분할 할당 => 이 아래에서는 반드시 구조분할할당 한 변수만 사용해야 한다.
               let {이용약관} = that.회원;  
               ...
               if( $(this).is(':checked')===true ){
                  ...
                  // that.회원.이용약관 = [];  // 빈 배열을 넣으면 배열이 삭제된다.
                  이용약관 = [];  // 빈 배열을 넣으면 배열이 삭제된다.
               }
            }
         });


      ///* 무료배송 정보수신동의 SMS, 이메일  체크박스 이벤트*///
         // 1. 2개를 체크하는 함수
         // 2. sns-check 체크박스 체인지 이벤트 : 2개를 체크하는 함수 호출 실행
         // 3. sns-check-all 체인지 이벤트 : 체크하면 2개 모두 선택,  체크 안하면 2개 모두 해제
      ...
      idCheckOk(){
         ...
         // 데이터베이스 조회하면 더이상 로컬스토레이지는 사용안함
         // 로컬스토레이지 회원 데이터 가져오기
         // [ 임시로 가져온 것-테스트 점검함 ]
         // for(let i=0; i<localStorage.length; i++){
         //   imsi = [...imsi, JSON.parse( localStorage.getItem(localStorage.key(i)) )];  // 키값을 가져오기
         // }

         // 아이디만 추출해서 중복 검사
         // 반복 아이디 비교 처리 그리고 아이디 비교 결과는 result 에 보관
         // 보관 result 안에 true, false를 최종 includes()로 비교 처리한다.
      ...
      submitFn(){  // 전송 메서드
         ...
         let cnt = 0;
            ...
            $('.submit-btn').on({
               ...
               // console.log( `that.회원 : ${that.회원.비밀번호}`);
               // console.log( `that.회원.아이디중복확인 : ${that.회원.아이디중복확인}` );
               // console.log( `that.회원.이메일중복확인 : ${that.회원.이메일중복확인}` );
               // console.log( `that.회원.이용약관 : ${that.회원.이용약관}` );
      ...
      //////////// /* 아이디, 이메일 중복확인, 휴대폰 인증확인 */ ///////////////
               ...
               let 마켓컬리회원 = {  // 테이블이름 market_kurly_member (필드1, 필드2, FIELD(ITEM == ATTRIBUTE ))
                  // idx : AUTO INCREMENT(자동증가인덱스번호), AI 유일성  11
                  아이디: that.회원.아이디,  // 16 id
                  비밀번호: that.회원.비밀번호,  // 16 pw
                  이름: that.회원.이름,    // 250 name
                  이메일: that.회원.이메일,   // 250 email
                  휴대폰: that.회원.휴대폰.replace(regExpHp, '$1-$2-$3'), // 13 hp 01079425305 => 치환(replace)  010-7942-5305
                 ...
                 // 인증번확인 입력상자, 버튼이 사라진다
                 // 인증번호받기 버튼이 다른번호 인증으로 바뀐다.
                 // 휴대폰 입력상자는 비활성화 입력불가상태
                 // 그러나 다른번호 인증 버튼을 크릭하면
                 // 입력상자 입력대가상태로 전환
...
///////////////////////////////////*  AJAX 전송  *//////////////////////////////////////////
               $.ajax({
                  ...
                  data: {  // PHP에게 보낼 데이터
                     ...
                     service: JSON.stringify(마켓컬리회원.이용약관),  // 문자열로 저장하기 stringify
                  ...
                  success: function(res){
                     that.회원.가입완료 = true;
                     $('.member-modal').fadeIn(300);
                     $('.modal-message').html(`감사합니다. ${마켓컬리회원.이름}님 회원가입을 축하드립니다.`);
                     // 모달창 닫고 인트로페이지로 이동하기
                  },
                  error: function(err){
